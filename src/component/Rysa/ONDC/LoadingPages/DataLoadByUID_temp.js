...
const DataLoadByUID = () => {
  ...
  const getONDCFormData = async () => {
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_REACT_APP_BASE_URL}getONDCFormDataDTO`,
        formData
      );
  ...
  useEffect(() => {
    if (
      Object.keys(lenders).length > 0 &&
      Object.keys(backendProducts).length > 0
    ) {
      handleLenders();
    }
  ...
  const callbacksLoadByMobileNumber = async (redirect) => {
    try {
      const formData = new FormData();
      formData.append("mobileNumber", mobileNumber);
      const response = await axios.post(
      ...
      if (response.status === 200) {
        ...
        if (response.data && response.data.length > 0) {
          console.log(
            ...
            response.data?.[0]?.context?.transaction_id
          );
  ...
  const handleMISStatus = async () => {
    try {
      ...
      const response = axios.post(
        `${process.env.NEXT_PUBLIC_REACT_APP_BASE_URL}api/mis/updateBeforeTransaction`,
        formData
      );
      if (response.status === 200) {
  ...
  const processData = async (data) => {
    try {
      ...
      setLenders(
        response.filter((item) => item.context.action === "on_search")
      );
      ...
      if (response.length > 0) {
        ...
        const selectResponse = response.filter(
  ...
  const handleWebSocketMessageForSelectManual = async (data) => {
    ...
    try {
      ...
      if (
        parsedData?.message?.order?.items?.[0]?.tags?.[0]?.list?.[0]?.descriptor
          ?.code === "CONSENT_HANDLER"
      ) {
        console.log("Entered in setting consent handler block");
        const consentHandler =
          parsedData?.message?.order?.items?.[0]?.tags?.[0]?.list?.[0]?.value ||
          null;
        console.log("setting consent handler : ", consentHandler);

        handleAARedirect(
          parsedData.context.transaction_id,
          consentHandler,
          mobileNumber,
          parsedData.context.bpp_id
        );
        ...
        if (
          ...
          const updatedPayload = {
            ...
            productName:
              parsedData?.message?.order.provider.descriptor?.name || "NA",
            //added afterwards
  ...
  const handleAARedirect = async (
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_REACT_APP_BASE_URL}finvuRedirect`,
        formData
      );
    ...
    if (lenders) {
      ...
      for (const lender of lenders) {
        if (
          ...
          const formId =
            lender.message.catalog.providers?.[0]?.items?.[0]?.xinput?.form?.id;
  ...
  const handleSelectApi = async (updatedPayload) => {
    try {
      ...
      console.log(
        "The value that we got from aa second select response is : ",
        selectResponse
      );
    } catch (error) {
      console.log(
        "error we are getting while calling select api from ondcList : ",
        error
      );
    }
  };
        ...
        for (const tempLender of lenders) {
          if (tempLender.context.bpp_id === lender.context.bpp_id) {
            ...
            if (matchedConsent) {
              ...
              router.push(
                `/ondc/loanapprovefetch?bppId=${lender.context.bpp_id}&transactionId=${lender.context.transaction_id}&mobileNumber=${mobileNumber}&minAmt=${minValue}`
              );
  ...
  const handleGetLoanClickForHdb = () => {
    ...
    router.push(`/yubi/Start?mobileNumber=${mobileNumber}`);
  };
  ...
  const getHdbProduct = async (mobilenumber) => {
    try {
      ...
      if (response.status === 200) {
        ...
        callbacksLoadByMobileNumber(redirect);
      } else {
        const redirect = true;
        callbacksLoadByMobileNumber(redirect);
      }
