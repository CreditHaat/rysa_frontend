...
try {
  ...
  console.error("Error saving transactionId and offerFlag to MIS:", misError);
}
  ...
  // ====================================================================================================
        ...
        console.log("error while hitting search api");
    }
}
        ...
        if (response.status === 200) {
            ...
            if (response.data && response.data.length > 0) {
                ...
                if (filteredResponse.length > 0) {
                    ...
                    const selectResponse = filteredResponse.filter(item => item.context.action === "on_select")
                    // setOnSelectResponses((prev) => [...prev, selectResponse]);

                    const firstTimestamp = filteredResponse[0].context?.timestamp || response[0].timestamp;
