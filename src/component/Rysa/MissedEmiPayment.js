...
//   useEffect(()=>{
//     getUpcomingInstallment();
//   },[])

//   const getUpcomingInstallment=async()=>{
//     try{
//       const response = await axios.post(`${process.env.NEXT_PUBLIC_REACT_APP_BASE_URL}fetchInstallmentByEndDate`,
//       { loanNumber: selectedLoanData.loanNumber });
//       if(response.status === 200){
//         console.log("upcoming installment response is :: ",response);
//       }
//     }catch(error){
//       console.log("error while gettting upcoming installment",error);
//     }
//   }
...
export default function MissedEmiPayment() {
  ...
  useEffect(() => {
    if (selectedLoanData?.loanNumber) {
      ...
      getUpcomingInstallment(selectedLoanData.DisbursedLoanId);
    }
  }, [selectedLoanData]);

  const getUpcomingInstallment = async (disbursedLoanId) => {
    try {
      // const response = await axios.post(
      //   `${process.env.NEXT_PUBLIC_REACT_APP_BASE_URL}fetchMissedEmiByLoan`,
      //   { loanNumber }
      // );
  ...
  const handleUpdatePartPayment = async () => {
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_REACT_APP_BASE_URL}update-missed-emi`,
        {
          ...
          // amount: amountToPay,
          // currency: "INR",
          paymentLabel: "MISSED_EMI_PAYMENT",
        ...
        !waitingForCallback ? (<> <div className={styles.container}>
          ...
          <form className={styles.card} onSubmit={handleSubmit}>
            ...
            <div className={styles.row}>
              <div className={styles.label}>Your EMI</div>
              <div className={styles.value}>₹{emiAmount.toLocaleString()} </div>
              <div>+ some late fee charges may apply</div>
            </div>
