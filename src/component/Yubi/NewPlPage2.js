...
// const loadCompanyOptions = debounce(fetchCompanyOptions, 300);
  ...
  const StoreDataToBackendForSalaried = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_REACT_APP_BASE_URL}PlApplyNew_SalariedHDB`,
        formData1
      );
            ...
            <>
              ...
              <div className={styles.formGroup}>
                ...
                {formErrors.loanAmount && (
                  <span loanAmount="error">{formErrors.loanAmount}</span>
                )}
              </div>
