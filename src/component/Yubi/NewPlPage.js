...
const NewPlPage = ({ params, searchParams }) => {
  ...
  const handleChange = (e) => {
    ...
    if (name === "fullname") {
      ...
      setFirstName(fname);
      setLastname(lname);
      setFatherName(fathername); // ensure you have this useState
  ...
  const validateForm = () => {
    ...
    if (!formData.fullname || formData.fullname.trim() === "") {
      ...
      if (nameParts.length < 3) {
        errors.fullname =
          "Please enter First Name, Father's Name, and Last Name";
        valid = false;
      }
    }
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_REACT_APP_BASE_URL}chfronetendotpgenerator_PlApplyNewHDB`,
        formData1
      );
  ...
  const apiExecutionBackend = async (productname, lenderCpi, productId) => {
    ...
    if (lenderCpi === 1) {
      console.log("inside lender cpi == 1");
      ...
      try {
        ...
        // console.log("before if hdb call");
        // if (productname === "HDB") {
        //   console.log("inside the hdb api");
        //   try {
        //     const response = await axios.get(
        //       `${process.env.NEXT_PUBLIC_REACT_APP_BASE_URL}applyHDBLoan`,
        //       {
        //         params: {
        ...
        //     console.log("before executing the code response");
        //     if (response.data.code === 0) {
        //       const combinedObj = response.data.obj;

        //       const client_loan_id = combinedObj.client_loan_id;
        //       const request_id = combinedObj.request_id;
        //       const redirection_url = combinedObj.redirection_url;

        //       console.log("🔗 Redirecting to:", redirection_url);

        //       // Redirect user to AA journey
        //       if (redirection_url) {
        //         window.location.href = redirection_url;
        //       }
        //     } else {
        //       console.log("❌ HDB flow failed:", response.data.msg);
        //     }
        //   } catch (err) {
        ...
        if (productname === "HDB") {
          try {
            ...
            if (createLeadResponse.data.code !== 0) {
              console.log(
            ...
            if (createResponse.data.code !== 0) {
              console.log("❌ Failed to create loan:", createResponse.data.msg);
              setRedirectionLinkLoader(false);
              return;
            }
            ...
            if (statusResponse.data.code !== 0) {
              // console.log("❌ Failed to get status:", statusResponse.data.msg);
              console.log("Failed to get response");
              setRedirectionLinkLoader(false);
              return;
            }

            // const status = statusResponse.data.obj.status;

            // if (status.toLowerCase() !== "success") {
            //   console.log("⚠️ Status is not success, skip AA:", status);
            //   setRedirectionLinkLoader(false);
            //   return;
            // }
            ...
            // const callbackStatus = localStorage.getItem("callbackStatus");

            // console.log(
            //   "The callbackStatus before initiateHDBAA is :: ",
            //   callbackStatus
            // );
            // // 🟢 STEP 3: Initiate AA
            ...
            //   }
            // );
            // console.log("✅ AA Response:", aaResponse.data);

            // if (aaResponse.data.code === 0) {
            ...
            //   }
            // } else {
            //   console.log("❌ Failed to initiate AA:", aaResponse.data.msg);
            // }
          } catch (err) {
